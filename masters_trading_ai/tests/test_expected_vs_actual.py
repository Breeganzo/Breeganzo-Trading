...
def test_api_expected_vs_actual_uses_close_for_today_after_hours(
    tmp_path: Path, monkeypatch
):
    logs_dir = tmp_path / "prediction_log"
    logs_dir.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(
        server,
        "_get_close_prices_for_date",
        lambda tickers, _date: {"ABC.NS": 105.0},
    )
    monkeypatch.setattr(
        server,
        "_get_live_prices_batch",
    ...
    monkeypatch.setattr(
        server.PredictionTracker,
        "check_outcomes",
        staticmethod(lambda _date: {}),
    )

    with server.app.test_client() as client:
        resp = client.get(f"/api/expected-vs-actual?date={today}")
        assert resp.status_code == 200
        payload = resp.get_json()

    assert payload["after_hours_mode"] is True
    row = payload["results"][0]
    assert row["actual_price"] == 105.0
    assert row["actual_close"] == 105.0
